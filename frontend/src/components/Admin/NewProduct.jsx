...
const NewProduct = () => {
  ...
  const [images, setImages] = useState([]); // dataURL images
  const [imagesPreview, setImagesPreview] = useState([]);

  const [logo, setLogo] = useState(""); // dataURL logo
  const [logoPreview, setLogoPreview] = useState("");

  // ==== Valet Key helpers ====

  // Gọi BE xin chữ ký upload Cloudinary
  const getUploadSignature = async (folder) => {
    const { data } = await axios.get(
      "/api/v1/upload/get_upload_signature",
      {
        params: { folder },
        withCredentials: true, // nếu backend dùng cookie token
      }
    );
  ...
  // Upload 1 ảnh (dataURL) lên Cloudinary, trả về { public_id, url }
  ...
  const handleLogoChange = (e) => {
    ...
    reader.onload = () => {
      if (reader.readyState === 2) {
        ...
        setLogo(reader.result); // dataURL để upload Cloudinary
      }
    };
    ...
    files.forEach((file) => {
      ...
      reader.onload = () => {
        if (reader.readyState === 2) {
          ...
          setImages((oldImages) => [...oldImages, reader.result]); // dataURL
        }
      };
  ...
  const newProductSubmitHandler = async (e) => {
    ...
    try {
      // 1) Upload brand logo lên Cloudinary (folder "brands")
      const uploadedLogo = await uploadToCloudinary(logo, "brands");

      // 2) Upload từng product image lên Cloudinary (folder "products")
      ...
      // 3) payload JSON gửi cho backend
      const payload = {
        name,
        description,
        price,
        ...
        highlights,
        specifications: specs, // [{title, description}]
      };

      // 4) gọi action (đã sửa để dùng JSON)
      dispatch(createProduct(payload));
    } catch (err) {
      console.error(err);
      enqueueSnackbar(
        err.message || "Error while uploading images",
        { variant: "error" }
      );
    }
  };
